...
## Findable, shareable, readable by agents

Three things added 2026-09-24, after the launch made their absence obvious.

**Share cards.** There were none, so a link posted anywhere was a line of grey
text beside the domain — visibly worse than kovati.dev's card in the same
thread. There is now a site-wide card and **one per page**, generated by Satori
at build from the page's own title and description, with the counts on the
site-wide one read from the manifest.

The per-page cards are a route (`app/og/[...slug]/route.tsx`) rather than an
`opengraph-image.tsx` beside the page, because Next refuses a metadata file
under an *optional* catch-all and `/docs/[[...slug]]` is one. Their paths end
in `.png` deliberately: a static export has no server to honour the
`Content-Type` the handler sets, so Firebase infers the type from the
extension, and an extensionless card would reach a scraper as
`application/octet-stream`.

**Analytics.** GA4 on the same property as kovati.dev — one property with two
hostnames beats two properties nobody compares, and a reader crossing between
them stays one session. Consent Mode v2, denied inline in `<head>` before
gtag.js is fetched, with the same banner and the same storage key as the
marketing site. The key is shared even though `localStorage` is per-origin and
these are two origins: it costs nothing and the answer carries over if the
sites are ever merged. Route changes are sent by hand, because on a
documentation site almost all the reading happens after the first load.

**For agents.** `/llms.txt` is an index — every page with its title,
description and URL, grouped by section, about 16 KB. `/llms-full.txt` is the
whole corpus, about 460 KB, read from the MDX on disk rather than the rendered
HTML.

The cleaning there is deliberately conservative: a `<Shot>` becomes its `alt`
text, because that alt was written to describe a screenshot to somebody who
cannot see it, which is exactly what a model needs and exactly what a naive JSX
strip would discard. Callouts keep their prose. Relative links are made
absolute, since the file is read on its own.

`robots.txt` and `sitemap.xml` did not exist either. Both do now, with the AI
crawlers named explicitly rather than left to the wildcard — the same list as
kovati.dev, for the same reason: we sell an agent-native suite, and a crawler
that is unsure about a site skips it.
